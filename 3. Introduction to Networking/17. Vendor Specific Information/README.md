...
1. Better Organization: Network administrators can group endpoints based on any common attribute they share.

2. Increased Security: Network segmentation disallows unauthorized members from sniffing network packets in other VLANs.

3. Simplified Administration: Network administrators do not have to worry about the physical locations of an endpoint.

4. Increased Performance: With reduced broadcast traffic for all endpoints, more bandwidth is made available for use by the network.
...
<h3> Assigning NICs a VLAN in Linux </h3>
...
@htb[/htb]$ sudo vconfig add eth0 20
...
@htb[/htb]$ sudo ip addr add 192.168.1.1/24 dev eth0.20

@htb[/htb]$ sudo ip link set up eth0.20

At last, we can check whether the interface has changed states to up:

@htb[/htb]$ ip a | grep eth0.20
...
<h3>Assigning NICs a VLAN in Windows</h3>
...
<h3>Analyzing VLAN Tagged Traffic</h3>
...
@htb[/htb]$ tshark -r "The Ultimate PCAP v20221220.pcapng" -T fields -e vlan.id | sort -n -u
...
<h3>Security Implications and VLAN Attacks</h3>
...
<h3>Cisco Discovery Protocol</h3>
